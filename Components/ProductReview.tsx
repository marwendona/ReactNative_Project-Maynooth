...
const ProductReview: React.FC<ProductReviewProps> = ({
  ...
  const renderRatingStars = (rating: number) => {
    ...
    for (let i = 0; i < filledStars; i++) {
      stars.push(<Icon key={i} iconName={'star_filled'} size='Large' color={theme.colors.yellow100}></Icon>); 
    }
    for (let j = 0; j < remainingStars; j++) {
      stars.push(<Icon key={filledStars + j} iconName={'star_filled'} size='Large' color={theme.colors.gray20}></Icon>);
    }
